module Voteable
  ...
  def current_vote(user)
    if self.up_voters.include? user.fb_id.to_s
      :up
    elsif self.down_voters.include? user.fb_id.to_s
      :down
    end
  end
  ...
  def vote_handler(current, vote, user, toggle = 0)
    if current == vote
      if (current && vote == :up) && toggle == 1
        vote_adjust(-1, 0, -1)
        self.up_voters.delete_if {|x| x == user.fb_id.to_s}
      elsif (current && vote == :down) && toggle == 1
        vote_adjust(0, 1, 1)
        self.down_voters.delete_if {|x| x == user.fb_id.to_s}
      end
    else
      if !current && vote == :up
        vote_adjust(1, 0, 1)
        self.up_voters << user.fb_id.to_s
      elsif !current && vote == :down
        vote_adjust(0, -1, -1)
        self.down_voters << user.fb_id.to_s
